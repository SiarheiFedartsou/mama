...
struct EmissionCost {
 public:
  explicit EmissionCost(double distance_weight = 0.03, double bearing_weight = 1.0)
      : distance_weight_(distance_weight), bearing_weight_(bearing_weight) {}
  double operator()(const Projection& projection, const Location& location) const {
    const auto distance_cost = -distance_weight_ * std::pow(projection.distance_m, 2.0);
    auto bearing_cost = 0.0;

    if (location.bearing) {
      const auto bearing_diff = AngleDiff(projection.bearing_deg, *location.bearing) / 180.0;
...
struct TransitionCost {
 public:
  explicit TransitionCost(double haversine_distance, double distance_diff_weight = 0.2)
      : haversine_distance_(haversine_distance), distance_diff_weight_(distance_diff_weight) {}
  std::optional<double> operator()(double path_distance) const {
    return -distance_diff_weight_ * std::abs(path_distance - haversine_distance_);
  }
...
MapMatcher::MapMatcher(std::shared_ptr<Graph> graph) : graph_(std::move(graph)) {}
...
Location MapMatcher::Update(const Location& input_location) {
  ...
  if (last_states_.empty()) {
    ...
    TransitionCost transition_cost_computer{input_location.coordinate.Distance(previous_location_->coordinate)};
    ...
    last_states_.clear();
    auto path_matrix = BuildPathMatrix(previous_candidate_points, candidate_points, graph_);

    for (size_t candidate_index = 0; candidate_index < candidate_points.size(); ++candidate_index) {
      ...
      for (size_t previous_candidate_index = 0; previous_candidate_index < previous_candidate_points.size();
           ++previous_candidate_index) {
        auto transition_cost = transition_cost_computer(path_matrix[previous_candidate_index][candidate_index]);
        if (!transition_cost) {
          continue;
        }
        auto cost = previous_candidate_sequence_costs[previous_candidate_index] + *transition_cost + emission_cost;
        if (cost > max_cost) {
          max_cost = cost;
        }
      }
...
Location MapMatcher::BuildResult(const Location& location, const std::vector<Projection>& candidates) {
  if (candidates.empty()) {
    return location;
  }
  ...
  for (size_t i = 1; i < candidates.size(); ++i) {
    if (last_states_[i].sequence_cost > last_states_[best_index].sequence_cost) {
      best_index = i;
    }
  }
...
void MapMatcher::RestoreState(const state::State& pbf_state) {
  ...
  for (const auto& pbf_hmm_state : pbf_state.hmm_states()) {
    ...
    state.point_on_graph = {
        {pbf_hmm_state.point_on_graph().edge_id().tile_id(), pbf_hmm_state.point_on_graph().edge_id().edge_index()},
...
void MapMatcher::SaveState(state::State& pbf_state) {
  ...
  for (const auto& state : last_states_) {
    ...
    pbf_hmm_state->mutable_point_on_graph()->mutable_edge_id()->set_tile_id(state.point_on_graph.edge_id.tile_id);
    pbf_hmm_state->mutable_point_on_graph()->mutable_edge_id()->set_edge_index(state.point_on_graph.edge_id.edge_index);
    pbf_hmm_state->mutable_point_on_graph()->set_offset(state.point_on_graph.offset);
    pbf_hmm_state->set_sequence_cost(state.sequence_cost);
  }
  assert(previous_location_);
  *pbf_state.mutable_previous_location() = ConvertLocationToProto<state::Location>(*previous_location_);
}

Location MapMatchingController::Update(const Location& location, std::string& state) {
...
Location MapMatchingController::Update(Location location) {
  if (previous_location_) {
    ...
    if (!location.bearing) {
      auto bearing = previous_location_->coordinate.BearingTo(location.coordinate);
      location.bearing = bearing;
    }
    if (!location.speed) {
      auto timestamp_diff = location.timestamp.ToSeconds() - previous_location_->timestamp.ToSeconds();
      auto distance = location.coordinate.Distance(previous_location_->coordinate);
      location.speed = distance / timestamp_diff;
    }
  }

  constexpr auto kMinSpeedMps = 1.0;
  if (location.speed && *location.speed < kMinSpeedMps && previous_matched_location_) {
...
void MapMatchingController::RestoreState(const state::State& state) {
  ...
  if (state.has_previous_location() && state.has_previous_matched_location()) {
    ...
    previous_matched_location_ = ConvertProtoToLocation(state.previous_matched_location());
  } else {
    previous_location_ = {};
...
void MapMatchingController::SaveState(state::State& state) {
  ...
  assert(previous_location_);
  assert(previous_matched_location_);
  *state.mutable_previous_location() = ConvertLocationToProto<state::Location>(*previous_location_);
  *state.mutable_previous_matched_location() = ConvertLocationToProto<state::Location>(*previous_matched_location_);
}
